...
def report_db_statistics(relations):
    """
    Report/Print the existing count of records in given list of relations.
    
    :param relations: A list of relation.
    """
    # loop over all the tables
    for table in relations:
        # relations are are known in globals.py to avoid SQL injection
        query = "SELECT COUNT(*) FROM " + table + ";"
        result = exec_get_one(query)
        # print the count of records in each relation. 
        if result:
            print("Table: ", table, " Records: ", result[0])